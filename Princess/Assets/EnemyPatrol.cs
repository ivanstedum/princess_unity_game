...
private void Update()
{
    ...
    isAttacking = gameObject.GetComponent<Enemy>().PlayerInSight();
    Debug.Log($"you should now see that player is in bounds: {isAttacking}");
    ...
    else if (playerTransform.position.x >= minX && playerTransform.position.x <= maxX &&
